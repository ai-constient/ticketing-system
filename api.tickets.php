...
class TicketApiController extends ApiController {
    ...
    */
    ...
    function getTicket($id) {
        ...
        if (!($ticket = Ticket::lookup(['number' => $id]))){
            return $this->exerr(500, __('Ticket not found'));
        };

        $ticketClone = clone $ticket;
        if (!empty($ticket->ht)) {  
            $subject = $ticketClone->getSubject();
            $ticket->ht['subject'] = $subject;
        }
    ...
    function createTicket($data, $source = 'API') {
        ...
        $required_fields = ['subject','name','message','email','department'];

        foreach($required_fields as $req_field){
            if (!array_key_exists($req_field, $data) || empty($data[$req_field])) {
                $this->exerr(500, $req_field . " is Required");
            }
        }

        $data['deptId'] = $data['department'];
